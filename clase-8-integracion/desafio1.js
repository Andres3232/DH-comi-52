...
// a
const catalogoCursos = [["html5", 4000] , ["css3", 5000], ["javascript", 10000], ["react", 7000], ["nodejs", 15000]];
...
// d
function facturar(nombre, apellido, catalogoCursos, cursosAlumno) {
    let monto = calcularPrecio(catalogoCursos, cursosAlumno);
...
// e
console.log(facturar("Facu", "Erbin", catalogoCursos, cursosElegidos));



    // cursosAlumno.map(curso => {
    //     const encontrado = catalogo.find(item => item[0].toUpperCase() === curso);
    //     if (encontrado) {
    //         precio += encontrado[1];
    //     }
    // });